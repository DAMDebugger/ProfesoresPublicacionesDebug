...
public class Principal {
	...
	private static int menu() {
		...
		System.out.println("--------------------------");
		int opcion = Utilidades.leerInt(1, 6); 
	...
	private static void nuevoProfe(ArrayList<Profesor> listaProfesores) {
		...
		if (EmailValidator.validarEmail(email)){ //Si el mail es correcto
			...
			if (!esta){ //Profe nuevo
				Profesor aux=null;
				// falta pedir al usuario resto de datos y rellenar aux
				aux.setEmail(email);
	...
	private static void listaProfes(ArrayList<Profesor> listaProfesores) {
		int contador;
		for (contador=0;contador<listaProfesores.size();contador++);	
			System.out.println(listaProfesores.get(contador).toString());
